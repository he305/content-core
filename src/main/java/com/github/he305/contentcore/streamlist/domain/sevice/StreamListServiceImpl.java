...
public class StreamListServiceImpl implements StreamListService {
    ...
    public void handleWatchingListContentAccountAddedEvent(WatchingListContentAccountAddedEvent event) {
        ContentAccount contentAccount = contentAccountExchangeService.getContentAccountById(event.getContentAccountId());
        if (!streamListPlatformValidator.isStreamChannel(contentAccount.getPlatform())) {
            return;
        }
